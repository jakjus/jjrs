...
export const getOrCreatePlayer = async (p: PlayerObject | PlayerAugmented): Promise<ReadPlayer> => {
  const auth = p.auth
  const playerInDb = await db.get('SELECT elo FROM players WHERE auth=?', [auth])
  if (!playerInDb) {
      await db.run('INSERT INTO players(auth, name, elo) VALUES (?, ?, ?)', [p.auth, p.name, 1200])
