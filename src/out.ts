...
const throwRealBall = async (game: Game, forTeam: TeamID, toPos: {x: number, y: number}, evCounter: number) => {
  ...
  //                       _..._
	...
	room.setDiscProperties(0, {x: toPos.x, y: toPos.y, radius: defaults.ballRadius, cMask: room.CollisionFlags.all, cGroup: room.CollisionFlags.ball | room.CollisionFlags.kick | room.CollisionFlags.score, invMass: defaults.ballInvMass })
